...
EsProject* es_proj_load(const char* proj_file) {
    ...
    ES_INFO("项目文件内容长度: %zu", strlen(content));
    ES_INFO("项目文件前200字符: %.200s", content);
    ...
    ES_INFO("成功加载项目: %s", project->name);
    if (project->description && project->description[0] != '\0') {
        ES_INFO("项目描述: %s", project->description);
    }
    ES_INFO("项目类型: %d", project->type);
    ...
    if (item_group) {
        ES_INFO("找到 ItemGroup: %s", item_group);
...
#ifdef DEBUG
        fprintf(fp, "    <DebugSymbols>%s</DebugSymbols>\n", prop->debug_symbols ? "true" : "false");
#endif
