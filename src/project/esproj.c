...
EsProject* es_proj_create(const char* name, EsProjectType type) {
    ...
    project->type = type;
    project->output_type = strdup(type == ES_PROJ_TYPE_LIBRARY ? "dll" : "exe");
    project->root_namespace = strdup(name);
    project->description = strdup("");


    project->sdk_version = strdup("1.0.0");


    time_t now = time(NULL);
    char time_str[64];
    strftime(time_str, sizeof(time_str), "%Y-%m-%d %H:%M:%S", localtime(&now));
    project->created_date = strdup(time_str);
    project->modified_date = strdup(time_str);
...
void es_proj_add_file(EsProject* project, const char* file_path, const char* item_type) {
    ...
    item->item_type = strdup(item_type ? item_type : "Compile");
...
void es_proj_add_dependency(EsProject* project, const char* name, const char* version, const char* path) {
    ...
    dep->version = strdup(version ? version : "*");
    dep->path = path ? strdup(path) : NULL;
...
int es_proj_create_template(EsProject* project, const char* output_dir) {
    ...
    if (!project->project_root) {
        project->project_root = strdup(output_dir);
    }
