...
EsIRValue es_ir_var(EsIRBuilder* builder, const char* name) {
    ...
    result.data.name = ES_STRDUP(name);
    return result;
}
...
EsIRValue es_ir_string_const(EsIRBuilder* builder, const char* str) {
    ...
    builder->module->string_constants[id] = ES_STRDUP(str);
...
void es_ir_module_destroy(EsIRModule* module) {
    ...
    while (func) {
        EsIRFunction* next = func->next;


        ES_FREE(func->name);
        if (func->param_count > 0 && func->params) {
            for (int i = 0; i < func->param_count; i++) {
                ES_FREE(func->params[i].name);
            }
            ES_FREE(func->params);
        }
        ...
        while (block) {
            EsIRBasicBlock* next_block = block->next;


                EsIRInst* inst = block->first_inst;
                while (inst) {
                    EsIRInst* next_inst = inst->next;


                    for (int i = 0; i < inst->operand_count; i++) {
                        if (inst->operands[i].type == ES_IR_VALUE_VAR && inst->operands[i].data.name) {
                            ES_FREE(inst->operands[i].data.name);
                        }
                    }
                    if (inst->result.type == ES_IR_VALUE_VAR && inst->result.data.name) {
                        ES_FREE(inst->result.data.name);
                    }


                    if (inst->operands) {
                        ES_FREE(inst->operands);
                    }
    ...
    for (int i = 0; i < module->string_const_count; i++) {
        ES_FREE(module->string_constants[i]);
    }
    ES_FREE(module->string_constants);

    if (module->globals) {
        for (int i = 0; i < module->global_count; i++) {
            ES_FREE(module->globals[i].name);
        }
...
EsIRFunction* es_ir_function_create(EsIRBuilder* builder, const char* name, EsIRParam* params, int param_count, EsTokenType return_type) {
    ...
    while (existing) {
        if (strcmp(existing->name, name) == 0) {
            ...
            if (existing->param_count == -1 && param_count >= 0) {
                ...
                if (param_count > 0) {
                    existing->params = ES_MALLOC(param_count * sizeof(EsIRParam));
                    for (int i = 0; i < param_count; i++) {
                        existing->params[i].name = ES_STRDUP(params[i].name);
    ...
    EsIRFunction* func = ES_CALLOC(1, sizeof(EsIRFunction));
    func->name = ES_STRDUP(name);
    ...
    if (param_count > 0) {
        func->params = ES_MALLOC(param_count * sizeof(EsIRParam));
        for (int i = 0; i < param_count; i++) {
            func->params[i].name = ES_STRDUP(params[i].name);
...
EsIRBasicBlock* es_ir_block_create(EsIRBuilder* builder, const char* label) {
    EsIRBasicBlock* block = ES_CALLOC(1, sizeof(EsIRBasicBlock));
    block->label = ES_STRDUP(label);
    return block;
}
...
static EsIRInst* add_instruction(EsIRBuilder* builder, EsIROpcode opcode) {
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "add_instruction called with opcode: %d", opcode);
    if (builder && builder->current_function) {
        ES_DEBUG_LOG("IR", "Current function: %s", builder->current_function->name);
    }
    if (builder && builder->current_block) {
        ES_DEBUG_LOG("IR", "Current block: %s", builder->current_block->label);
    }
    #endif
    if (!builder->current_block) {
        #ifdef DEBUG
        ES_DEBUG_LOG("IR", "No current block, creating new one");
        #endif
        char label[32];
    ...
    EsIRInst* inst = ES_CALLOC(1, sizeof(EsIRInst));
    inst->opcode = opcode;
    inst->operand_capacity = 4;
    inst->operands = ES_CALLOC(inst->operand_capacity, sizeof(EsIRValue));

    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "Instruction created at: %p, opcode: %d", (void*)inst, opcode);
    if (builder && builder->current_function) {
        ES_DEBUG_LOG("IR", "Instruction will be added to function: %s", builder->current_function->name);
    }
    if (builder && builder->current_block) {
        ES_DEBUG_LOG("IR", "Instruction will be added to block: %s", builder->current_block->label);
    }
    #endif
    ...
    if (!builder->current_block->first_inst) {
        ...
        #ifdef DEBUG
        ES_DEBUG_LOG("IR", "First instruction added to block, first_inst: %p, last_inst: %p",
                    (void*)builder->current_block->first_inst, (void*)builder->current_block->last_inst);
        #endif
    } else {
        builder->current_block->last_inst->next = inst;
        builder->current_block->last_inst = inst;
        #ifdef DEBUG
        ES_DEBUG_LOG("IR", "Instruction appended to block, new last_inst: %p", (void*)builder->current_block->last_inst);
        #endif
    }
...
static void add_operand(EsIRInst* inst, EsIRValue operand) {
    if (inst->operand_count >= inst->operand_capacity) {
        ...
        inst->operands = ES_REALLOC(inst->operands, inst->operand_capacity * sizeof(EsIRValue));
    }
...
void es_ir_store(EsIRBuilder* builder, const char* name, EsIRValue value) {
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "es_ir_store called for variable: %s, value type: %d", name, value.type);
    if (builder && builder->current_function) {
        ES_DEBUG_LOG("IR", "Current function: %s", builder->current_function->name);
    }
    if (builder && builder->current_block) {
    ...
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "Instruction created at address: %p", (void*)inst);
    #endif
    add_operand(inst, es_ir_var(builder, name));
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "Variable operand added: %s", name);
    #endif
    add_operand(inst, value);
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "Value operand added, type: %d", value.type);
    if (value.type == ES_IR_VALUE_TEMP) {
        ES_DEBUG_LOG("IR", "Temp value index: %d", value.data.index);
    }
    #endif
}
...
void es_ir_alloc(EsIRBuilder* builder, const char* name) {
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "es_ir_alloc called for variable: %s", name);
    #endif
    ...
    add_operand(inst, es_ir_var(builder, name));
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "Operand added for variable: %s", name);
    #endif
}
...
void es_ir_jump(EsIRBuilder* builder, EsIRBasicBlock* target) {
    ...
    if (inst) {
        ...
        inst->operands[0].data.name = ES_STRDUP(target->label);
    }
}
...
EsIRValue es_ir_call(EsIRBuilder* builder, const char* func_name, EsIRValue* args, int arg_count) {
    ...
    inst->result = es_ir_temp(builder);
    #ifdef DEBUG
    ES_DEBUG_LOG("IR", "es_ir_call returning result type: %d", inst->result.type);
    #endif
    return inst->result;
}
...
void es_ir_print(EsIRModule* module, FILE* output) {
    ...
    while (func) {
        ...
        while (block) {
            ...
            while (inst) {
                fprintf(output, "    ");
                es_ir_print_instruction(inst, output, func);
                fprintf(output, "\n");
                inst = inst->next;
            }
