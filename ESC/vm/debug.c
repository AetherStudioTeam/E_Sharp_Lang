...
void es_disassemble_chunk(EsChunk* chunk, const char* name) {
    ...
    for (int offset = 0; offset < chunk->count;) {
        offset = es_disassemble_instruction(chunk, offset);
    }
}
...
static int constant_instruction(const char* name, EsChunk* chunk, int offset) {
    uint8_t constant = chunk->code[offset + 1];
    printf("%-16s %4d '", name, constant);
    es_value_print(chunk->constants.values[constant]);
    printf("'\n");
    return offset + 2;
}
