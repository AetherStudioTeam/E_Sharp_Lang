...
#define ES_PLATFORM_H
    ...
    #define es_write_file_win32(handle, buffer, count, written, overlapped) \
        do { DWORD _written; WriteFile(handle, buffer, count, &_written, overlapped); *(written) = _written; } while(0)
    #define es_read_file_win32(handle, buffer, count, read, overlapped) \
        do { DWORD _read; ReadFile(handle, buffer, count, &_read, overlapped); *(read) = _read; } while(0)
#else
    #include <sys/syscall.h>
    #define es_write_console(handle, buffer, count, written) \
        (*(written) = write(handle, buffer, count))
    #define es_read_console(handle, buffer, count, read_result, ctrl) \
        (*(read_result) = read(handle, buffer, count))
    #define es_write_file_posix(handle, buffer, count, written, overlapped) \
        (*(written) = write(handle, buffer, count))
    #define es_read_file_posix(handle, buffer, count, read_result, overlapped) \
        (*(read_result) = read(handle, buffer, count))
#endif
    ...
    static inline int es_platform_create_directory(const char* path) { return CreateDirectory(path, NULL) ? 0 : -1; }
    ...
    #define es_platform_create_directory(path) mkdir(path, 0755)
